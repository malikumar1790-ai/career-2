// Enhanced Contact Form API with Comprehensive Error Handling and Security
// Stores submissions in Supabase and sends dual emails (admin + user confirmation)
...
export default async function handler(req, res) {
  ...
  try {
    console.log('📧 Enhanced Contact API: Processing request...');
    ...
    try {
      ...
      if (dbError) {
        ...
        console.log('✅ Contact submission stored in database:', submissionId);
      }
    } catch (dbError) {
    ...
    try {
      ...
      return res.status(500).json({
        ...
        timestamp: new Date().toISOString(),
        error: process.env.NODE_ENV === 'development' ? verifyError.message : 'Email service connection failed'
      });
    }
      ...
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 700px; margin: 0 auto; background: #f8fafc;">
          ...
          <div style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); padding: 40px 30px; text-align: center; border-radius: 12px 12px 0 0;">
            ...
            <p style="color: #e5e7eb; margin: 10px 0 0 0; font-size: 18px;">New Contact Form Submission</p>
          </div>
          ...
          <div style="background: white; padding: 40px 30px; border-radius: 0 0 12px 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            ...
            <div style="background: #f1f5f9; padding: 25px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #3b82f6;">
              <h3 style="color: #1e3a8a; margin: 0 0 20px 0; font-size: 22px; border-bottom: 2px solid #3b82f6; padding-bottom: 10px;">👤 Contact Information</h3>
              ...
              <div style="display: grid; gap: 15px;">
                ...
                ${submissionId ? `
                <div style="display: flex; align-items: center;">
                  <span style="font-weight: 600; color: #374151; min-width: 120px;">🆔 ID:</span>
            ...
            <div style="background: #fefefe; border: 2px solid #e5e7eb; padding: 25px; border-radius: 10px; margin-bottom: 25px;">
              <h3 style="color: #1e3a8a; margin: 0 0 15px 0; font-size: 20px;">💬 Message:</h3>
            ...
            <div style="border-top: 2px solid #e5e7eb; padding-top: 20px; text-align: center; color: #6b7280; font-size: 14px;">
              ...
              <p style="margin: 5px 0 0 0;"><strong>Client IP:</strong> ${clientIP}</p>
            </div>
          </div>
        </div>
      `,
      text: `
        🚀 NEXARIZA AI - NEW CONTACT FORM SUBMISSION
        ==========================================
        
        ⚡ NEW LEAD ALERT - Respond within 24 hours!
    ...
    try {
      ...
      const userResult = await transporter.sendMail(userEmailData);
